...
import RegulationLegalityNotice from "./RegulationLegalityNotice";
import { OFFICIAL_VGC_HANDBOOK_URL } from "../utils/regulation";
...
const RegulationSelector = ({ compact = false }) => {
  ...
  return (
    <div className="regulation-selector-block">
      <RegulationLegalityNotice compact={compact} />
      <div className={`regulation-selector card-surface${compact ? " regulation-selector-compact" : ""}`}>
        ...
        >
          {regulations.map((entry) => (
            <option key={entry.id} value={entry.id}>
              ...
              {entry.legalityUnverified && !entry.isPlaceholder ? " (unverified)" : ""}
        ...
        {!compact && (
          <p className="regulation-selector-hint">
            Saved with the active team. {regulation.notes} · Max {regulation.maxRestricted ?? 2}{" "}
            Restricted. Verify against the{" "}
            <a
